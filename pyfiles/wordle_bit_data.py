...
def exclude(wordstring, bitstring, arr):
    ...
    for i in range(len(char_bit)):
        ...
        n_bit = char_bit.count((char, "1")) + char_bit.count(
            (char, "2")
        )  # says n instances of some character exists
        n_0_bit = char_bit.count(
            (char, "0")
        )  # says there can't be more than n instances of that character
        ...
        if bit == "0" and n_bit == 0:  # grey
            ...
            excluded_list.extend(
                [word for word in arr if char not in word or word[i] == char]
            )
        elif bit == "2":  # green
...
if __name__ == "__main__":
    ...
    nxn_bitstring = []
    with open("venv/04 personal/wordle/data/wordle_bit_output.csv", "r") as csvfile:
        csv_reader = csv.reader(csvfile)
        for row in csv_reader:
            nxn_bitstring.append(row)
    with open("venv/04 personal/wordle/data/wordle_dict.txt", "r") as textfile:
        wordle_dataset = textfile.readlines()

    n_words = len(wordle_dataset)
    for i in range(n_words):
        wordle_dataset[i] = wordle_dataset[i].strip()
    print(n_words)
    nxn_big_data = [[None for _ in range(n_words)] for _ in range(n_words)]
    nxn_shanon = [None for _ in range(n_words)]
    for i in range(n_words):
        for j in range(n_words):
            nxn_big_data[i][j] = (wordle_dataset[j], nxn_bitstring[i][j])

    for i in range(n_words):
        with Pool(processes=5) as p:
            print(nxn_big_data[i][i])
            answer_row = p.starmap(
                shanon_bits,
                [
                    (guess, bitstring, wordle_dataset)
                    for _, (guess, bitstring) in nxn_big_data
                ],
            )
        nxn_shanon[i] = answer_row
        print(i / n_words * 100, "%")
